...
namespace SLywnow
{
	...
	public class SaveSystemAlt : MonoBehaviour
	{
		...
		public static float GetFloat(string key, float def = 0)
		{
			...
			if (ss_sl.name.Contains(key)) if (ss_sl.type[ss_sl.name.IndexOf(key)] == SaveSystemSL.SSLTpe.floatS || ss_sl.type[ss_sl.name.IndexOf(key)] == SaveSystemSL.SSLTpe.undefinedS)
				{
